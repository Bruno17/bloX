...
class bloxf {
    ...
    function displaydatas($outerdata = array()) {
        ...
        //replace innerrows
        $daten = str_replace($searches,$replaces,$this->parser->template);
        
        //echo str_replace('[[+','[ [+',$daten);
        //echo '<pre>' . print_r($this->parser->getPlaceholders(), 1) . '</pre>';
        $this->parser->template = $daten;
        $daten = $this->parser->Render($this->bloxconfig['parseLazy']);
        unset($this->parser);
        if ($cache == '1') {
    ...
    //renderdatarows
    /////////////////////////////////////////////////
    function renderdatarows($rows, $tpl, $rowkey = '', $outerdata = array(), $outerpath='') {
        ...
        if (is_array($rows)) {
            ...
            foreach ($rows as $row) {
                ...
                $out[] = $this->renderdatarow($row, $tpl, $rowkey, $outerdata, $rowscount, $iteration, $keypath, $outerpath);
                $iteration++;
            }
        }
    ...
    /////////////////////////////////////////////////
    function renderdatarow($row, $rowTpl = 'default', $rowkey = '', $outerdata = array(), $rowscount = 0, $iteration = 0, $keypath='', $outerpath='') {
        ...
        */
        ...
        //$datarowTplData['iteration'] = $iteration + 1;
        ...
        //echo $keypath . '<br />';
        //echo $rowkey . ',';
        $output = str_replace($searches,$replaces,$tpl->template);
        
        $output = str_replace('[[+++','[[+'.$keypath,$output);
